...
dataset = read.csv('Dataset_WorkersSurevy.csv')
...
#Scatterplot
plot(logwage_h ~ experience, data=dataset)

#Correlation test
cor.test(dataset$logwage_h , dataset$experience)

# Fitting Simple Linear Regression to the model
regressor = lm(formula = wage_h  ~ experience,
               data = dataset)
